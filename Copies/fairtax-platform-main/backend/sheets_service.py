...
def log_to_the_5_sheet(referrer_name, ref_data):
    """Save referrer and their 5 referrals to 'The 5' sheet.

    Args:
        referrer_name: Name of person entering referrals
        ref_data: Dict with ref_name_1-5, ref_phone_1-5

    Saves to "The 5" sheet with columns: name, referrals
    """
    ws = _sheet("The 5")
    if ws is None:
        print(f"[THE_5] Sheets unavailable")
        return False

    try:
        # Ensure headers exist
        headers = ["name", "referrals"]
        _ensure_headers(ws, headers)

        # Build referrals dictionary from ref_name_1-5 and ref_phone_1-5
        referrals_dict = {}
        for i in range(1, 6):
            ref_name = ref_data.get(f"ref_name_{i}", "").strip()
            ref_phone = ref_data.get(f"ref_phone_{i}", "").strip()

            # Only include if both name and phone are provided
            if ref_name and ref_phone:
                referrals_dict[f"ref_{i}"] = {
                    "name": ref_name,
                    "phone": ref_phone
                }

        # Save to sheet: name | referrals (as JSON string)
        row = [referrer_name, json.dumps(referrals_dict)]
        _ws_call(ws, 'append_row', row, value_input_option="USER_ENTERED")

        print(f"[THE_5] [OK] Saved {len(referrals_dict)} referrals for {referrer_name}")
        return True

    except Exception as e:
        print(f"[THE_5] [ERROR] Error: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def process_approved_submissions_for_quotes():
    """
    Process APPROVED submissions and generate quote links.

    For each row where:
    - approval_status = "APPROVED"
    - quote_link is empty

    Generate quote URL and save to quote_link column.

    Returns:
        dict: {"processed": count, "errors": count, "skipped": count}
    """
    ws = _sheet("Submissions")
    if ws is None:
        print("[SHEETS] process_approved_submissions: Sheets unavailable")
    ...
    try:
        all_values = _ws_call(ws, 'get_all_values')
        if len(all_values) <= 1:
            print("[SHEETS] process_approved_submissions: No data rows")
            return {"processed": 0, "errors": 0, "skipped": 0}

        headers = all_values[0]
        approval_col_idx = headers.index('approval_status') if 'approval_status' in headers else None
        quote_link_col_idx = headers.index('quote_link') if 'quote_link' in headers else None
        submission_id_col_idx = headers.index('submission_id') if 'submission_id' in headers else None

        if approval_col_idx is None or quote_link_col_idx is None or submission_id_col_idx is None:
            print("[SHEETS] process_approved_submissions: Required columns not found")
            return {"processed": 0, "errors": 0, "skipped": 0}
        ...
        for row_idx, row in enumerate(all_values[1:], start=2):
            try:
                # Get values
                approval_status = row[approval_col_idx] if approval_col_idx < len(row) else ""
                quote_link = row[quote_link_col_idx] if quote_link_col_idx < len(row) else ""
                submission_id = row[submission_id_col_idx] if submission_id_col_idx < len(row) else ""

                # Skip if not APPROVED or already has quote_link
                if approval_status.strip() != "APPROVED":
                    skipped += 1
                    continue

                if quote_link and quote_link.strip():
                    skipped += 1
                    continue

                # Generate quote link
                quote_url = f"https://fairtax-backend.onrender.com/api/download/quote_{submission_id}.pdf"

                # Update sheet
                _ws_call(ws, 'update_cell', row_idx, quote_link_col_idx + 1, quote_url)
                print(f"[SHEETS] ✅ Generated and saved quote link for {submission_id}")
                processed += 1

            except Exception as e:
                print(f"[SHEETS] ❌ Error processing row {row_idx}: {e}")
                errors += 1
                continue

        print(f"[SHEETS] process_approved_submissions: Processed={processed}, Errors={errors}, Skipped={skipped}")
        return {"processed": processed, "errors": errors, "skipped": skipped}

    except Exception as e:
        print(f"[SHEETS] process_approved_submissions ERROR: {e}")
        import traceback
        traceback.print_exc()
