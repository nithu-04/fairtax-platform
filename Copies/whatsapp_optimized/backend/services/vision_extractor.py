...
def extract_pass1_vision(image_bytes_list, doc_type):
    ...
    try:
        ...
        # ✅ OPTIMIZATION: Reduce pages per doc type (most data in first few pages)
        DOC_TYPE_MAX_PAGES = {
        ...
        if total_pages > MAX_PAGES:
            logger.debug(f"Large document ({total_pages} pages). Processing first {MAX_PAGES} pages only.")
        ...
        def _extract_single_page(page_data):
            ...
            try:
                ...
                non_null_count = sum(1 for v in fields.values()
                                    if isinstance(v, dict) and v.get("value") not in (None, "", 0, "0"))

                return (page_num, result, non_null_count)
            except Exception as e:
                logger.debug(f"Page {page_num} error: {str(e)}")
                return (page_num, None, 0)

        # Process pages in parallel with early stopping
        page_results = []
        page_errors = []
        ...
        with ThreadPoolExecutor(max_workers=min(4, pages_to_process)) as executor:
            ...
            for future in as_completed(futures):
                ...
                if result is None or non_null_count == 0:
                    ...
                    # Early stop if many consecutive blanks
                    if consecutive_blanks >= MAX_CONSECUTIVE_BLANKS and page_results:
                        logger.debug(f"Early stop: {consecutive_blanks} consecutive blanks")
        ...
        logger.error(f"Vision extraction error: {str(e)}", exc_info=True)
        raise
