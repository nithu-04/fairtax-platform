    ...
    try:
        ...
        if not submission_id:
            ...
            # 🔥 CASE 3: normal update
                ...
                # Always update timestamp when updating existing row
                from datetime import datetime
                data["timestamp"] = datetime.now().isoformat()
                sheets_service.update_row(row, data)

        # [OK] HOOK: Save to "The 5" sheet when user enters 5 referrals (free filing)
        # When a user submits their 5 referrals, save to "The 5" sheet
        try:
            is_free = data.get('filing_category') == 'free' or data.get('filing_type') == 'free'
            print(f"[THE_5] is_free={is_free}")

            if is_free:
                # Check if user entered at least one referral
                has_referrals = any(
                    data.get(f"ref_name_{i}") and data.get(f"ref_phone_{i}")
                    for i in range(1, 6)
                )
                print(f"[THE_5] has_referrals={has_referrals}")
                print(f"[THE_5] Referral data: ref_name_1={data.get('ref_name_1')}, ref_phone_1={data.get('ref_phone_1')}")

                if has_referrals:
                    referrer_name = data.get('name', 'Unknown')
                    print(f"[THE_5] Calling log_to_the_5_sheet with name={referrer_name}")
                    success = sheets_service.log_to_the_5_sheet(referrer_name, data)
                    if success:
                        print(f"[THE_5] [OK] Saved referrals for {referrer_name} to 'The 5' sheet")
                    else:
                        print(f"[THE_5] [WARN] Warning: Could not save to 'The 5' sheet")
        except Exception as e:
...
def submit():
    try:
        ...
        # Notify FairTax Admin of new submission — non-blocking, errors don't block submission
        try:
            admin_phone = "9600165254"  # FairTax WhatsApp number
            user_name = data.get("name", "User")
            user_phone = data.get("phone", "N/A")
            filing_category = data.get("filing_category", "regular").upper()

            admin_msg = f"Hi FairTax,\n\n I just submitted a filing"

            wa_resp = whatsapp_service.send_text(admin_phone, admin_msg)
            print(f"[WA][ADMIN] Notification sent to FairTax: {wa_resp}")
        except Exception as wa_err:
            print(f"[WA][ADMIN] WARNING - Notification failed (non-blocking): {wa_err}")
            # Continue with submission regardless — notification is secondary
...
@app.route("/api/process-approved-quotes", methods=["POST"])
def process_approved_quotes():
    """
    Process APPROVED submissions and generate quote links.

    For each row where approval_status = "APPROVED" and quote_link is empty,
    generates the quote URL and saves it to the quote_link column in Google Sheets.

    This endpoint is called after admin approves a submission in Google Sheets.
    """
    try:
        logger.info("[PROCESS_QUOTES] Starting quote processing...")
        result = sheets_service.process_approved_submissions_for_quotes()

        logger.info(f"[PROCESS_QUOTES] Completed: {result}")

        return jsonify({
            "success": True,
            "message": "Quote links processed",
            "result": result
        }), 200

    except Exception as e:
        logger.error(f"[PROCESS_QUOTES] Error: {e}")
        logger.error(f"[PROCESS_QUOTES] Traceback: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
