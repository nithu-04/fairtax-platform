...
def compress_image_for_vision(image_bytes, max_width=2000, quality=85):
    """
    Compress image before sending to Vision API for faster processing.

    Vision API doesn't need ultra-high resolution. This reduces:
    - File size (faster upload)
    - API processing time
    - Cost (slightly smaller payload)

    Args:
        image_bytes: Raw image bytes
        max_width: Max width in pixels (default 2000, Vision can handle this)
    ...
    try:
        ...
        # Vision API doesn't benefit from > 2000px width, but needs enough clarity
        ...
        logger.warning(f"Image compression failed: {str(e)}, returning original image")
        return image_bytes
