...
def extract_from_text(text, doc_type="payslip"):
    ...
    try:
        ...
        # ✅ OPTIMIZATION: Pre-extract easy fields with regex (avoid Vision API for simple fields)
        regex_result, regex_meta = deterministic_extract(text, doc_type)
        regex_confidence = sum(v.get('confidence', 0) for v in regex_meta.get('fields', {}).values()) / max(1, len(regex_meta.get('fields', {})))

        # If regex got high confidence on key fields, use it (faster, avoid Vision API)
        ...
        if key_fields_found >= 3 and regex_confidence > 0.8:
            ...
            # Use Vision API for better accuracy
