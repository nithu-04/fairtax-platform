...
function showJokerModal() {
  ...
  if (codeEl) {
    ...
    codeEl.textContent = "Get Your Code At The End!";
  }
    ...
    try {
      if (filingType === "regular") {
        ...
        if (!referralCode && !localStorage.getItem("referral_code")) {
          try {
            const s1 = collectStep(1);
            // Referral flow uses name/phone, not name/phone
            const actualName = s1.name || s1.name;
            const actualPhone = s1.phone || s1.phone;
...
$("#submit").onclick = async () => {
  ...
  try {
    ...
    if (j.success) {
      ...
      // 🎉 Auto-open WhatsApp with confirmation message
      try {
        const waMessage = "Hi Fairtax! Just submitted my filing";
        const waUrl = `https://wa.me/9600165254?text=${encodeURIComponent(waMessage)}`;
        setTimeout(() => {
          window.open(waUrl, '_blank');
        }, 500); // Small delay so thank you page displays first
      } catch (e) {
        console.log("WhatsApp could not be opened:", e);
      }
    } else {
      alert("Error: " + j.error);
    }
