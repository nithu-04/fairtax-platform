...
const WHATSAPP_PHONE = '919600165254'; // FairTax WhatsApp number
function openWhatsAppMessage(formData) {
  try {
...
- Income Category: ${formData.category || 'Regular'}
- Submission ID: ${formData.submission_id || 'N/A'}

Please update me on my filing status and send the quote when ready.

Thanks!`;

    const whatsappLink = `https://wa.me/${WHATSAPP_PHONE}?text=${encodeURIComponent(message)}`;
    console.log('[DEBUG] Opening WhatsApp link:', whatsappLink);
    const result = window.open(whatsappLink, '_blank');

    if (!result) {
      console.error('[ERROR] WhatsApp popup was blocked by browser');
    } else {
      console.log('[SUCCESS] WhatsApp opened successfully');
    }
  } catch (e) {
    console.error('[ERROR] Failed to open WhatsApp:', e);
  }
}
...
$("#submit").onclick = async () => {
  ...
  try {
    ...
    if (j.success) {
      // 🔔 WHATSAPP INTEGRATION: Open WhatsApp immediately on successful submission
      const formDataForWhatsApp = {
        name: all.name,
        pan: all.pan,
        category: all.filing_category || 'Regular',
        submission_id: all.submission_id
      };
      openWhatsAppMessage(formDataForWhatsApp);
